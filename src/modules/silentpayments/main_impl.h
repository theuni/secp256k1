...
static void secp256k1_silentpayments_calculate_input_hash(unsigned char *input_hash, const unsigned char *outpoint_smallest, secp256k1_ge *pubkey_sum) {
...
static int secp256k1_silentpayments_create_t_k(secp256k1_scalar *t_k_scalar, const unsigned char *shared_secret33, const unsigned int k) {
    secp256k1_sha256 hash;
    ...
    /* It is statistically improbable that we will ever find a set of inputs such that t_k exceeds the
     * curve order, but we still handle this case explicitly as specified by BIP352
     */
    return !overflow;
}

static int secp256k1_silentpayments_create_output_pubkey(const secp256k1_context *ctx, secp256k1_xonly_pubkey *P_output_xonly, const unsigned char *shared_secret33, const secp256k1_pubkey *recipient_spend_pubkey, const unsigned int k) {
     ...
     */
    if (!secp256k1_silentpayments_create_t_k(&t_k_scalar, shared_secret33, k)) {
        return 0;
    }
...
int secp256k1_silentpayments_sender_create_outputs(
    ...
    size_t n_recipients,
    const unsigned char *outpoint_smallest,
    const secp256k1_keypair * const *taproot_keypairs,
    size_t n_taproot_keypairs,
    ...
    ARG_CHECK((plain_seckeys != NULL) || (taproot_keypairs != NULL));
    if (taproot_keypairs != NULL) {
        ARG_CHECK(n_taproot_keypairs > 0);
    } else {
        ARG_CHECK(n_taproot_keypairs == 0);
    }
    ...
    for (i = 0; i < n_plain_seckeys; i++) {
        ...
        /* We can declassify return value, because scalar set only fails if the seckey is invalid */
    ...
    for (i = 0; i < n_taproot_keypairs; i++) {
        secp256k1_ge addend_point;
        ret = secp256k1_keypair_load(ctx, &addend, &addend_point, taproot_keypairs[i]);
...
int secp256k1_silentpayments_recipient_create_label(const secp256k1_context *ctx, secp256k1_pubkey *label, unsigned char *label_tweak32, const unsigned char *recipient_scan_key, const unsigned int m) {
    secp256k1_sha256 hash;
...
int secp256k1_silentpayments_recipient_create_output_pubkey(const secp256k1_context *ctx, secp256k1_xonly_pubkey *P_output_xonly, const unsigned char *shared_secret33, const secp256k1_pubkey *recipient_spend_pubkey, const unsigned int k)
{
    VERIFY_CHECK(ctx != NULL);
